...
module.exports = function(answers) {
  ...
  const privatePath = path.join(projectPath, "private");
  ...
  files.push({path: path.join(projectPath, tsconfig.name), content: tsconfig.content});
  dependencies.push(...tsconfig.dependencies);

  // types/index.d.ts 模板
  const indexType = require("./templates/type");
  files.push({path: path.join(typesPath, indexType.name), content: indexType.content});
...
`;
  ...
  init.put(exhandler.importSegments, exhandler.initSegments);
  ...
  // 其它模板列表规格化
  let templatesSelected = ['validator','database', 'redis', 'auth', 'rabbitmq', 'websocket', 'socketio', 'nacos'];
  ...
  if (JSON.stringify(templatesSelected) != '[]') {
    for (const templateSelected of templatesSelected) {
      ...
      if (template.importSegments) {
        init.put(template.importSegments,template.initSegments);
      }
  ...
  packageJson.scripts = {
      ...
      "restart": "npx tsc --outDir dist && node dist/index",
...
`;
  ...
  // evpconfig.json
  const evpconfigPath = path.join(projectPath, 'evpconfig.json');
  let evpconfigJson = {
    lang: 'TypeScript'
  }

  files.push({path: evpconfigPath, content: JSON.stringify(evpconfigJson, null, 2)});
  ...
  console.log(chalk.blue(`$ cd ${answers.name}`));
  console.log(chalk.blue(`$ npm run restart`));
}