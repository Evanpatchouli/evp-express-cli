...
module.exports = function(templateSelected) {
  // 规格化
  let templates = ['validator', 'zod', 'database', 'redis', 'auth', 'rabbitmq', 'websocket', 'socketio', 'nacos'];
  let devtools = ['jest', 'pkg', 'pm2', 'eslint', 'babel'];
  if (!templates.includes(templateSelected) && !devtools.includes(templateSelected)) {
